...
<div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
    ...
    <div class="mb-6 grid grid-cols-2 gap-4">
        <div>
            ...
            <p><span class="font-semibold">تاریخ:</span> {{ $invoice['Date'] ?? '-' }}</p>
        </div>
        <div>
            <p><span class="font-semibold">مشتری:</span> {{ $invoice['ContactTitle'] ?? '-' }}</p>
            <p><span class="font-semibold">جمع کل:</span> {{ number_format($invoice['Sum'] ?? 0) }} تومان</p>
        </div>
    </div>
            ...
            <tbody>
                @foreach ($invoice['InvoiceItems'] ?? [] as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-right">{{ number_format($item['UnitPrice']) }}</td>
                        <td class="border border-gray-300 px-4 py-2 text-right">{{ number_format($item['TotalAmount']) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="mt-6 text-right">
        <p class="font-semibold text-lg">مجموع کل: {{ number_format($invoice['Sum'] ?? 0) }} تومان</p>
    </div>
</div>
@endsection
