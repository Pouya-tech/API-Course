...
@section('content')
    ...
    <main class="container mx-auto px-4 py-8 max-w-xl">
        ...
        <form method="POST" action="{{ route('posts.store') }}" class="bg-white p-6 rounded shadow space-y-4">
            @csrf
            ...
            <x-form.input lable="title" name="title" LayoutClass="mb-4" value="{{ old('title') }}" />
            {{-- @dump($errors->all()) --}}
            ...
            <x-form.input lable="category" name="category" LayoutClass="mb-4" value="{{ old('category') }}" />
            ...
            <div>
                ...
                <textarea name="description" rows="5" class="w-full mt-1 px-4 py-2 border rounded focus:ring"
                    placeholder="write post description">{{ old('description') }}</textarea>
            </div>
